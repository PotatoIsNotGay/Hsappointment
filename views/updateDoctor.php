        ...
        <div class="container card float-start mt-1 py-3 mx-2" style="background-color:#759e97;">
            ...
            <div class="card-body">
                <form action="../actions/updateDoc.php" method="post" enctype="multipart/form-data">
                    <label for="doctor_id" class="form-label text-light">Select Doctor:</label>
                    <?php
                    // Fetch doctor's name based on ID
                    $doctor_id = $_GET['id'];
                    $sql = "SELECT id, name FROM doctors WHERE id = :id";
                    ...
                    <input type="text" class="form-control" value="<?php echo $doctor['name']; ?>" disabled>
                    <br>
                    <label for="image" class="form-label text-light">New Image:</label>
                    <input type="file" name="image" id="image" class="form-control">
                    <br>
                    <label for="name" class="form-label text-light">New Name:</label>
                    <input type="text" name="name" id="name" class="form-control">
                    <br>
                    <label for="age" class="form-label text-light">New Age:</label>
                    <input type="text" name="age" id="age" class="form-control">
                    <br>
                    <label for="qualification" class="form-label text-light">New Qualification:</label>
                    <input type="text" name="qualification" id="qualification" class="form-control">
                    <br>
                    <label for="specialists" class="form-label text-light">New Specialist Type:</label>
                    <select class="form-select form-select-striped-md" name="specialists" id="specialists">
                        <?php
                    ...
                    <br>
                    <label for="sch1" class="form-label text-light">New Schedule Time 1:</label>
                    <select class="form-select form-select-striped-md" name="sch1" id="sch1">
                        <?php
                    ...
                    <br>
                    <label for="sch2" class="form-label text-light">New Schedule Time 2:</label>
                    <select class="form-select form-select-striped-md" name="sch2" id="sch2">
                        <?php
                        $sql3 = "SELECT * FROM schedules2";
                        $result3 = $conn->query($sql3);
                        if(isset($result3)){
                            while($row3 = $result3->fetch()){
                                echo "<option value='".$row3['id']."'>".$row3['time']."</option>";
                            }
                        }
                        ?>
                    </select>
